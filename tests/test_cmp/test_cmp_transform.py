...
class TestCmpTransform(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.nmface_test = create_test_default(nmface_file, CMP.nm2nmface, nm=cls.nm_test)
        cls.vuface_test = create_test_default(vuface_file, CMP.nmface2vuface, nmface=cls.nmface_test)
        cls.xyz_face_test = create_test_default(xyz_file, CMP.vuface2xyz_face, vuface=cls.vuface_test)
        cls.ea_face_test = create_test_default(ea_file, CMP.nm2ea_face, nm=cls.nm_test)

    def test_cmp2mn_face(self):
        with self.subTest('Testing nm2nmface.'):
            nmface = CMP.nm2nmface(nm=self.nm_test)
            self.assertTrue(np.array_equal(self.nmface_test, nmface), 'Error in nm2nmface()')

        with self.subTest('Testing nmface2nm_face.'):
            nm, face = CMP.nmface2nm_face(nmface=self.nmface_test)
            self.assertTrue(np.array_equal(self.nm_test, nm), 'Error in nmface2nm_face()')

        with self.subTest('Testing reversion.'):
            nm, face = CMP.nmface2nm_face(nmface=nmface)
