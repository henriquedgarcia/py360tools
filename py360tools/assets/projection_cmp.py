...
class CMP(ProjectionBase):
    def nm2xyz(self, nm):
        nmface = nm2nmface(nm=nm, proj_shape=self.canvas.shape)
        vuface = nmface2vuface(nmface=nmface, proj_shape=self.canvas.shape)
        xyz, face = vuface2xyz_face(vuface=vuface)
        return xyz

    def xyz2nm(self, xyz):
        vuface = xyz2vuface(xyz=xyz)
        nmface = vuface2nmface(vuface=vuface, proj_shape=self.canvas.shape)
        cmp, face = nmface2nm_face(nmface=nmface, proj_shape=self.canvas.shape)
        return cmp
